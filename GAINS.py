...
#setup environment
env.workspace="F:\\MissouriDo\\GeneralSearch\\EPA\\Data_Weather\\August_6.gdb" 
...
#num=168 # Enter the total time to run the script 
num=577  # 24 day=576 hrs
p_t=[]
p_v=[]
p_wa=[]
Elevation=Raster("to")
...
for num in range (1,num,1):
  ...
  ## to update the infilitration ratef_rate.save(outpath + "\\f_rate"+ str(num))
  f_rate= Con(t <= tp,Con((p>0) | (exc>= f_rate*(t-to)),p,0),Con((p>0) | (exc>= f_rate*(t-to)),f_rate,0)) 
  f_rate.save(outpath + "\\f_rate" + str(num))
  print "1- The infiltration f_rate"+ str(num)+ " has been computed successfully"
  ...
  fd.save(outpath + "\\fd" + str(num))
  print"2- The accu.infiltration depth fd"+ str(num)+ " has been computed successfully"
  ...
  ETd=Con(p==0,Con(wc<=wp,ETd,Con(watermask3==0,ETd+Et0*(t-to),Con(watermask3==1,ETd+Et1*(t-to),Con(watermask3==2,ETd+Ev*(t-to))))),ETd)
  ETd.save(outpath + "\\ETd" + str(num))
  print"3- The accu.evapotranspiration depth ETd"+ str(num)+ " has been computed successfully"

  ## ## #4- fd-ETd= dw is the accumuated ifilitration- evapotranspiration  depth in (cm.)
  dw=fd-ETd
  dw.save(outpath + "\\dw" + str(num))
  print"4- The accu.(infilitraion - evapotranspiration) depth dw"+ str(num)+ " has been computed successfully"
  ...
  wc.save(outpath + "\\wc"+ str(num))
  print"5- The (wc) water content spatially  wc"+ str(num)+ "  has been computed successfully" 
  ...
  print"6- The Drainage depth Dr"+ str(num)+ " has been computed successfully"
  ...
  #exc=Con(p==0,Con(AcRain-fd>=0,Con(AcRain-fd-Ev*(t-to)>=0,AcRain-fd-Ev*(t-to),AcRain-fd),exc),exc)
  #exc=Con(p==0,Con(AcRain-fd>=0,Con(AcRain-fd-Ev*(t-to)>=0,AcRain-fd-Ev*(t-to),AcRain-fd),exc),exc)
  
  #exc=Con(p==0,Con(AcRain-fd>=0,Con(AcRain-fd-Ev*(t-to)>=0,Con(watermask3==0,AcRain-fd-Ev*(t-to),Con(watermask3==1,AcRain-fd-Ev*(t-to),Con(watermask3==2,AcRain-fd,AcRain-fd)))),exc),exc)
  #exc=Con(p==0,Con(AcRain-fd>=0,Con(AcRain-fd-Ev*(t-to)>=0,Con((watermask3==0)|(watermask3==1),AcRain-fd-Ev*(t-to),Con(watermask3==2,AcRain-fd,AcRain-fd))),exc),exc)
  exc=Con(p==0,Con(AcRain-fd>=0,Con(AcRain-fd-Ev*(t-to)>=0,Con((watermask3==0)|(watermask3==1),AcRain-fd-Ev*(t-to),AcRain-fd)),exc),exc)
  exc.save(outpath + "\\exc"+ str(num))
  print"7-a The net accu. excess depth (cm)on the surface  exc"+ str(num)+ " has been computed successfully"
  excm= exc/100
  excm.save(outpath + "\\excm"+ str(num))
  print" -b The net accu. excess depth (m)on the surface  excm"+ str(num)+ " has been computed successfully" 
  ...
  vzone.save(outpath + "\\vzone"+ str(num))
  print"8-a The net accu. zone excess volume on the surface vzone"+ str(num)+ " has been computed successfully"

  # Execute ZonalStatistics
  Vol = ZonalStatistics("D", "value", "excm"+ str(num), "SUM","NODATA")
  WaElev=Vol/6578.0+ minElevation
  #Volume=Vol
  #print"Vol=",Vol, "WaElev=",WaElevarc
  
  # Save the output
  Vol.save(outpath + "\\Vol"+ str(num))
  print" -b The (Vol) total excess volume in m3 for the wetland has been computed successfully"

  #p_v.append(Volume)
  WaElev.save(outpath + "\\WaElev"+ str(num))
...
##  Elevation=Times("WaElev"+str(num),1000)
##
##  # take integer of the elevation in mm
##  outInt=Int(Elevation)
##  # Save the output
##  outInt.save(outpath + "\\outInt"+ str(num))
##  
##  #############################################################
##  #Search cursor
##  fcc = outpath+ "\\"+ "outInt"+str(num)
##  field = "Value"
